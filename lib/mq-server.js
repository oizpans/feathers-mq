...
module.exports = function getFn(params = {}) {
  return function subscribeNats() {
    ...
    nats.subscribe(`${app.get('name')}.*.create`, async (request, res, subject) => {
      const [appName, serviceName, methodName] = subject.split('.');
      debug(`${methodName} request for ${app.get('name')}.${serviceName}`);
      const req = JSON.parse(request);
      ...
      try {
        app.service(serviceName).on('created', data => nats.publish(`${appName}.${serviceName}.created.events`, JSON.stringify(data)));
    ...
    nats.subscribe(`${app.get('name')}.*.patch`, async (request, res, subject) => {
      ...
      try {
        app.service(serviceName).on('patched', data => nats.publish(`${appName}.${serviceName}.patched.events`, JSON.stringify(data)));
    ...
    nats.subscribe(`${app.get('name')}.*.update`, async (request, res, subject) => {
      ...
      try {
        app.service(serviceName).on('updated', data => nats.publish(`${appName}.${serviceName}.updated.events`, JSON.stringify(data)));
    ...
    nats.subscribe(`${app.get('name')}.*.remove`, async (request, res, subject) => {
      ...
      try {
        app.service(serviceName).on('removed', data => nats.publish(`${appName}.${serviceName}.removed.events`, JSON.stringify(data)));
