  ...
  return function subscribeNats() {
    ...
    nats.subscribe(`${app.get('name')}.*.create`, async (request, res, subject) => {
      ...
      try {
        ...
        // .on('created',
        // data => nats.publish(`${appName}.${serviceName}.created.events`, JSON.stringify(data)));
    ...
    nats.subscribe(`${app.get('name')}.*.patch`, async (request, res, subject) => {
      ...
      try {
        // app.service(serviceName)
        // .on('patched',
        // data => nats.publish(`${appName}.${serviceName}.patched.events`, JSON.stringify(data)));
    ...
    nats.subscribe(`${app.get('name')}.*.update`, async (request, res, subject) => {
      ...
      try {
        // app.service(serviceName)
        // .on('updated',
        // data => nats.publish(`${appName}.${serviceName}.updated.events`, JSON.stringify(data)));
    ...
    nats.subscribe(`${app.get('name')}.*.remove`, async (request, res, subject) => {
      ...
      try {
        // app.service(serviceName)
        // .on('removed',
        // data => nats.publish(`${appName}.${serviceName}.removed.events`, JSON.stringify(data)));
