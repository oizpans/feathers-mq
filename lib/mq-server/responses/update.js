const debug = require('debug')('feathers-mq:server:update');
const Errors = require('@feathersjs/errors');
const wrapError = require('../wrap-error');

module.exports = async function update(request, replyTo, subject) {
  const { nats, app } = this;
  ...
  try {
    const result = await app.service(serviceName).update(request.id, request.data, request.params);
    if (replyTo) {
